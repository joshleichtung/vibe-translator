...
- [ ] Install Playwright (AC: 1)
  ...
  - [ ] Select TypeScript configuration
  - [ ] Install browser binaries (Chromium only for hackathon)
  - [ ] Verify Playwright CLI works: `npx playwright --version`
- [ ] Configure Playwright (AC: 2)
  - [ ] Update `playwright.config.ts` with correct web server config
  - [ ] Set baseURL to `http://localhost:8420`
  - [ ] Configure web server command: `npm run dev`
  - [ ] Set test timeout to 95s (90s API + 5s buffer)
  - [ ] Enable audio permissions for audio context tests
- [ ] Create test directory structure (AC: 3)
  - [ ] Create `tests/e2e/` directory
  - [ ] Move default example tests to `tests/e2e/`
  - [ ] Create subdirectories if needed (smoke/, integration/)
- [ ] Write smoke test (AC: 4)
  - [ ] Create `tests/e2e/smoke.spec.ts`
  - [ ] Test: App loads and shows title
  - [ ] Test: Vibe input field renders
  - [ ] Test: Generate button visible
  - [ ] Test: Pastel horror background color applied
- [ ] Add npm scripts (AC: 5, 6)
  - [ ] Add `"test:e2e": "playwright test"`
  - [ ] Add `"test:e2e:ui": "playwright test --ui"`
  - [ ] Add `"test:e2e:headed": "playwright test --headed"`
  - [ ] Run `npm run test:e2e` and verify passes
- [ ] Verify Playwright UI mode (AC: 6)
  - [ ] Run `npm run test:e2e:ui`
  - [ ] Verify Playwright UI opens
  - [ ] Run smoke test in UI mode
  - [ ] Verify test execution visible
